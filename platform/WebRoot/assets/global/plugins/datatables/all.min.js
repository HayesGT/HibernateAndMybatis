...
$.fn.dataTable.pipeline = function ( opts ) {
    ...
    return function ( request, drawCallback, settings ) {
        var ajax          = false; //是否ajax请求，一般都是读取的缓存
        var requestStart  = request.start;//请求数据开始的数目
        var requestLength = request.length;//每页的数据量,默认10
        var requestEnd    = requestStart + requestLength; //请求的数据结束的数目
         
        if ( settings.clearCache ) {//清楚缓存
