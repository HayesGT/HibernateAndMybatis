...
var mDialogCallback;
 ...
 /**  基于artDialog v6 重写的alert
      alert("简单的使用");
      alert("带回调方法的使用",function(opt){
         if(opt){
             alert("点击了确定方法继续往下走");
             //do something
         }else{
      ...
      alert("这个是一个复杂的使用,3秒后自动关闭","",'标题！',true,true,3000);
  * @param message 消息内容
  * @param callBack 回调方法，可以为""或者null
  * @param title 消息标题
  * @param showModal  是否遮罩，true遮罩，false不遮罩
  * @param confirm 是否显示取消按钮
  * @param time 自动关闭的时间
  * @param h 宽度
  * @param w 高度
  */
 window.alert = function(message,callBack,title,showModal,confirm,time,h,w,functionStr){
	 if(!title){
		 title="消息提示";
	 }
	 var alertHtml='<div id="AlertMessageBody"  class="msgbody">'+message+"</div>";
	 var d ;
	 if(!w){
		 w=300;
	 }
	 ...
	 function show(){
		 ...
		 $(".ui-dialog-content").removeClass("ui-dialog-content").addClass("ui-dialog-content-extend");
	     if(time){
	    	 $(".ui-dialog-footer").remove();
	    	 setTimeout(function () {
	             d.close().remove();
	             if(functionStr){
	            	 eval("'"+functionStr+"'");
 ...
 * 定时关闭的提示框
 * @param msg  消息提示
 * @param time 自动关闭的时间  2000（2秒）
 */
function alertAutoClose(msg,time,functionStr){
	alert(msg,"","","","",time,"","",functionStr);
}
/**重写 alert end*/
