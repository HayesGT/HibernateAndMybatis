...
public class Util {
	 ...
	 *@return  Object
	 */
	public static Object createDynamicBean(Map<String,Object> queryMap) {
		Iterator<String> iterQuery = queryMap.keySet().iterator();// 初始化bean 主要是处理int
		// double的默认值
		BeanGenerator bg = new BeanGenerator(); // 基于配置信息，利用cglib，动态创建类
		Map<String,Object> defaltSetMap = new HashMap<String,Object>();// 处理int double
		// 因为基于cglib，创建bean的时候，会自动赋值为0
		while (iterQuery.hasNext()) {
			String key =  iterQuery.next();
			String name = key;
